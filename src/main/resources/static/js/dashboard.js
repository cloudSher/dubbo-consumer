...
$(function () {
    ...
    var init_option = function(dt){
        var arr = JSON.parse(dt);
        sort(arr);
        console.log(arr);
        for(var i in arr){
            var msg = arr[i];
            x_data.push(timestamp2Date(msg.msgTm));
            series_data.push(msg.score);
        }
    ...
    var sort = function(arr){
        for(var i = 1; i < arr.length; i++){
            var k = i;
            var tmp = arr[i];
            while(k > 0 && tmp.msgTm < arr[k - 1].msgTm){
               arr[k] = arr[k - 1];
               k--;
            }
            arr[k] = tmp;
    ...
    var create_tbody = function (dt) {
        ...
        for(var i in arr){
            ...
            tbody += ''+'<tr>' +
                '<td>' + (i+1)+ '</td>'+
                '<td>' + timestamp2Date(msg.msgTm) + '</td>'+
                '<td>' + msg.xps + '</td>'+
                '<td>' + msg.clk + '</td>'+
                '<td>' + msg.cc + '</td>'+
                '<td>' + msg.xps_max + '</td>'+
                '<td>' + msg.clk_max + '</td>'+
                '<td>' + msg.cc_max + '</td>'+
                '<td>' + msg.time_cycle + '</td>'+
                '<td>' + msg.xps_factor + '</td>'+
                '<td>' + msg.clk_factor + '</td>'+
                '<td>' + msg.cc_factor + '</td>'+
                '<td>' + msg.time_factor + '</td>'+
                '<td>' + msg.xps_value + '</td>'+
                '<td>' + msg.clk_value + '</td>'+
                '<td>' + msg.cc_value + '</td>'+
                '<td>' + msg.time_value + '</td>'+
                '<td>' + msg.score + '</td>'+
                '</tr>'
        }
        $("#msg").append(tbody);
    ...
    $(".send").on("click",function () {
        var d = {num:$("input[name='num']").val(),xps:$("input[name='xps']").val(),clk:$("input[name='clk']").val()
            ,'cc':$("input[name='cc']").val(),'xps_max':$("input[name='xps_max']").val(),'clk_max':$("input[name='clk_max']").val()
            ,'cc_max':$("input[name='cc_max']").val(),'xps_factor':$("input[name='xps_factor']").val(),'clk_factor':$("input[name='clk_factor']").val()
            ,'cc_factor':$("input[name='cc_factor']").val(),'time_cycle':$("input[name='time_cycle']").val(),'time_factor':$("input[name='time_factor']").val()
        }
        // alert(d.num);
        $.ajax({
            type: 'POST',
